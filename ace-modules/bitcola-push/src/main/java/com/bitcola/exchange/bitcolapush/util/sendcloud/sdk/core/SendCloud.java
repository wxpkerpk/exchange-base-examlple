package com.bitcola.exchange.bitcolapush.util.sendcloud.sdk.core;


import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import com.bitcola.exchange.bitcolapush.util.sendcloud.sdk.config.Config;
import com.bitcola.exchange.bitcolapush.util.sendcloud.sdk.config.Credential;
...
import com.bitcola.exchange.bitcolapush.util.sendcloud.sdk.util.Md5Util;
import com.bitcola.exchange.bitcolapush.util.sendcloud.sdk.util.ResponseData;
import com.bitcola.exchange.bitcolapush.util.sendcloud.sdk.model.Attachment;
import com.bitcola.exchange.bitcolapush.util.sendcloud.sdk.model.MailAddressReceiver;
...
import com.bitcola.exchange.bitcolapush.util.sendcloud.sdk.model.TextContent;
...
public class SendCloud {
	 ...
	 * @throws IOException
	 */
	private ResponseData post(Credential credential, SendCloudMail mail) throws ClientProtocolException, IOException, ParseException {
			...
			if (mail.getTo().useAddressList()) {
				...
				if (!mail.getContent().useTemplate() && receiver.isBroadcastSend()) {
					...
					if (mail.getBody().getXsmtpapi() != null && !mail.getBody().getXsmtpapi().containsKey("to")) {
						mail.getBody().addXsmtpapi("to", JSONArray.parseArray(JSONObject.toJSONString(receiver.getTo())));
					}
				}
			}
		}
	 ...
	 * @throws SmsException
	 */
	public ResponseData sendSms(SendCloudSms sms) throws ClientProtocolException, IOException, SmsException, ParseException {
		...
		ResponseData result = new ResponseData();
			try {
				...
				if (json.containsKey("statusCode")) {
					result.setStatusCode(json.getInteger("statusCode"));
